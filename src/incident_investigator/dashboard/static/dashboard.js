...
function renderIncidentWorkspace() {
  ...
  const tabs = isArchive
    ? `<button class="tab-button active" data-tab="archive-overview">Bundle overview</button><button class="tab-button" data-tab="evidence">Evidence</button><button class="tab-button" data-tab="source">Source analysis</button><button class="tab-button" data-tab="chain">Causal chain</button><button class="tab-button" data-tab="fixes">Recommended fixes</button><button class="tab-button" data-tab="artifacts">Artifacts</button><button class="tab-button" data-tab="timeline-events">Timeline</button><button class="tab-button" data-tab="report">Report</button><button class="tab-button" data-tab="activity">Activity</button>`
    : `<button class="tab-button active" data-tab="evidence">Evidence</button><button class="tab-button" data-tab="source">Source analysis</button><button class="tab-button" data-tab="chain">Causal chain</button><button class="tab-button" data-tab="fixes">Recommended fixes</button><button class="tab-button" data-tab="report">Report</button><button class="tab-button" data-tab="log">Log viewer</button><button class="tab-button" data-tab="activity">Activity</button>`;
  root.innerHTML = `
...
function renderWorkspaceTab(tab) {
  ...
  const report = full?.report;
  const source = full?.source_analysis || archive?.source_analysis || analysis.source_analysis;
  if (tab === 'archive-overview') {
    ...
    if (!source) {
      target.innerHTML = '<div class="empty-state"><strong>No source analysis available</strong>This incident was created before source correlation was enabled or the selected analysis did not include it.</div>';
    } else {
      const statusClass = source.status === 'resolved' ? 'success' : source.status === 'multiple_candidates' ? 'medium' : 'neutral';
      const location = [source.repository, source.path].filter(Boolean).join(' / ') || source.path || 'No verified repository location';
      const lines = source.start_line ? `Lines ${source.start_line}${source.end_line && source.end_line !== source.start_line ? `–${source.end_line}` : ''}` : 'Line not identified';
      const githubAction = source.source_url ? `<a class="secondary button-link" href="${esc(source.source_url)}" target="_blank" rel="noreferrer">View source ↗</a>` : '';
      const candidates = source.candidates || [];
      const hints = source.inferred_hints || [];
      target.innerHTML = `<div class="source-analysis"><div class="source-summary"><div><div class="source-status-row"><span class="badge ${statusClass}">${esc(titleCase(source.status))}</span><span class="badge neutral">${Math.round((source.confidence || 0) * 100)}% confidence</span></div><h3>${esc(location)}</h3><p class="page-subtitle">${esc(source.explanation || source.lookup_message || 'Source location was derived from the available incident evidence.')}</p></div>${githubAction}</div><div class="meta-grid source-meta"><div class="meta-item"><span>Repository</span><strong>${esc(source.repository || 'Not verified')}</strong></div><div class="meta-item"><span>Path</span><strong>${esc(source.path || 'Not identified')}</strong></div><div class="meta-item"><span>Function</span><strong>${esc(source.function || 'Not identified')}</strong></div><div class="meta-item"><span>Location</span><strong>${esc(lines)}</strong></div><div class="meta-item"><span>Provider</span><strong>${esc(titleCase(source.provider || 'log'))}</strong></div><div class="meta-item"><span>Match</span><strong>${esc(source.match_reason || 'Log inference only')}</strong></div></div><div class="grid-2"><div class="action-card"><div class="action-title"><span>Why this code is relevant</span><span class="badge neutral">AI + evidence</span></div><p>${esc(rc?.probable_root_cause || archive?.probable_root_cause || analysis.probable_root_cause || source.explanation || 'Review the source context against the incident evidence.')}</p></div><div class="action-card"><div class="action-title"><span>Suggested correction direction</span><span class="badge medium">Review first</span></div><p>${esc(fix?.recommended_strategy || archive?.long_term_actions?.[0] || analysis.recommended_actions?.[0] || 'No specific code change has been generated for this incident.')}</p></div></div>${source.snippet ? `<div class="source-code-wrap"><div class="source-code-title">Relevant source context</div><pre class="source-code">${esc(source.snippet)}</pre></div>` : ''}${candidates.length > 1 ? `<div class="source-candidates"><h3>Other matching locations</h3><div class="action-list">${candidates.slice(1).map(item=>`<div class="action-card"><div class="action-title"><span>${esc(item.repository)} / ${esc(item.path)}</span><span class="badge neutral">${Math.round((item.confidence || 0) * 100)}%</span></div><p>${esc(item.match_reason)}</p>${item.source_url ? `<a href="${esc(item.source_url)}" target="_blank" rel="noreferrer">Open source ↗</a>` : ''}</div>`).join('')}</div></div>` : ''}${!source.repository && hints.length ? `<div class="source-candidates"><h3>Locations inferred from the log</h3><div class="action-list">${hints.map(item=>`<div class="action-card"><div class="action-title"><span>${esc(item.path || item.filename || 'Unknown file')}</span><span class="badge neutral">${Math.round((item.confidence || 0) * 100)}%</span></div><p>${esc(item.function ? `Function ${item.function}` : 'Function not identified')}${item.line_number ? ` · line ${item.line_number}` : ''}</p></div>`).join('')}</div></div>` : ''}</div>`;
    }
  } else if (tab === 'chain') {
  ...
  try {
    ...
    root.innerHTML = `${pageHeader('Workspace configuration', 'Settings', 'Inspect active AI, database, storage, and security configuration. Secrets are never exposed here.')}
      <div class="settings-grid">
        ...
        <section class="panel"><div class="panel-header"><h2>Source correlation</h2><span class="badge ${health.github_source_lookup === 'enabled' ? 'success' : 'neutral'}">${esc(titleCase(health.github_source_lookup))}</span></div><div class="panel-body"><div class="setting-row"><span>GitHub / GHE lookup</span><strong>${esc(titleCase(health.github_source_lookup))}</strong></div><div class="setting-row"><span>Fallback</span><strong>Log file/function/line inference</strong></div><div class="setting-row"><span>Repository access</span><strong>Read only</strong></div><p class="form-help">Credentials are server-side only and are never returned by this endpoint.</p></div></section>
